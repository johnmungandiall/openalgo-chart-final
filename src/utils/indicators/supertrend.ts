...
import { OHLCData, SupertrendPoint } from './types';
 ...
 * @returns Array of {time, value, color, trend} where trend is 1 (bullish) or -1 (bearish)
 */
export function calculateSupertrend(
  ...
  let prevTrend: 1 | -1 = 1; // 1 = bullish, -1 = bearish
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const atrVal = atrValues[i] as number;
